...
const CatagoresNews = () => {
  ...
  if (numericId === 0) {
    ...
    filterData = datas.filter((item) => item.others?.is_today_pick === true);
  } else {
    filterData = datas.filter(
      (item) => parseInt(item.category_id) === numericId
    );
  }
    ...
    <div className="p-4">
      ...
      {filterData.map((item) => (
        <div key={item.id} className=" p-4 rounded-lg shadow-md border mb-6">
          {/* Author and Controls */}
          <div className="flex justify-between p-4 items-center mb-3 bg-gray-200">
